...
- **Data Interval/Period**: Choose granularity and lookback period (e.g., 15m for 1mo).
...
## Troubleshooting
- **No Data Fetched**:
  - Ensure the ticker is valid (e.g., AAPL, RELIANCE.NS).
  - Use larger intervals (e.g., 15m) for longer periods (e.g., 1mo).
  - Check Streamlit logs for `yfinance` errors (e.g., HTTP 403).
- **Requirements Error**:
  - Verify `requirements.txt` matches the listed versions.
  - If errors persist, check Streamlit Cloud logs under **Manage App** > **Logs**.
- **Local Testing**:
